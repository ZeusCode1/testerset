...
exports.run = async (bot, message, args) => {
  ...
  if (mesaj == "sıfırla" || mesaj == "reset") {
    if (!leaveData)
      return message
        .reply({
          content: `> ❌ **Başarısız!** Zaten davet çıkı mesaj ayarlanmamış.`,
          allowedMentions: { repliedUser: false },
        })
        .catch((err) => {});
    ...
    message
      .reply({
        content: `> ✅ **Başarılı!** Davet çıkış mesaj sıfırlandı!`,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
  ...
  if (mesaj.length < 10 || mesaj.length > 1000)
    return message
      .reply({
        content: `> ❌ **Başarısız!** En az 10 karakter, en fazla 1000 karakter belirtebilirsin. **Örnek:** \`${prefix}çıkış-mesaj -üye- Görüşürüz! Seni -davetçi- davet etmişti!\``,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
