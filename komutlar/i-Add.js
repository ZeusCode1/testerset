...
exports.run = async (bot, message, args) => {
  ...
  if (!sayi)
    return message
      .reply({
        content: `> ❌ **Başarısız!** Lütfen eklenecek davet sayısını belirtin! **Örnek:** \`${prefix}davet-ekle @üye 10\``,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
  ...
  message
    .reply({
      content: `> ✅ **Başarılı!** ${member} adlı kişiye **${sayi}** davet eklendi! **Toplam daveti:** \`${
        data ? data : `0`
      }\``,
      allowedMentions: { repliedUser: false },
    })
