...
exports.run = async (bot, message, args) => {
  ...
  if (mesaj == "sıfırla" || mesaj == "reset") {
    if (!joinData)
      return message
        .reply({
          content: `> ❌ **Başarısız!** Zaten davet girdi mesaj ayarlanmamış.`,
          allowedMentions: { repliedUser: false },
        })
        .catch((err) => {});
    ...
    message
      .reply({
        content: `> ✅ **Başarılı!** Davet girdi mesaj sıfırlandı!`,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
  ...
  if (mesaj.length < 10 || mesaj.length > 1000)
    return message
      .reply({
        content: `> ❌ **Başarısız!** En az 10 karakter, en fazla 1000 karakter belirtebilirsin. **Örnek:** \`${prefix}giriş-mesaj -üye- Hoşgeldin! Seni davet eden -davetçi-\``,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
  ...
  let embed = new MessageEmbed()
    ...
    .addField(
      ...
      mesaj
        ...
        .replace("-davet_kod-", `aNDXQdK`)
        .replace("-davet_url-", `https://discord.gg/aNDXQdK`)
        ...
        .replace("-invite_code-", `aNDXQdK`)
        .replace("-invite_url-", `https://discord.gg/aNDXQdK`)
