...
exports.run = async (client, message, args) => {
  ...
  if(!user) return message.reply({ content: "Kimin uyarılarına bakmak istiyor isen onu etiketlemelisin veya kullanıcı hesap id yazmalısın!"})
 const warns = await db.get(`${message.guild.id}${user.id}.warns`)
 if(!warns) return message.reply({ content: "Bu kullanıcıya ait uyarı bulunmamaktadır!"})
